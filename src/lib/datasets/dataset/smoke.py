...
import json
import os
from src.tools.reval import dets
import torch.utils.data as data


class Smoke(data.Dataset):
    num_classes = 1
    ...
    def __init__(self, opt, split):
        ...
        self.data_dir = os.path.join(opt.data_dir, 'smoke')
        self.img_dir = os.path.join(self.data_dir, 'image')
        if split == 'train':
            self.annot_path = os.path.join(
                self.data_dir, 'annotations', 'train_smoke.json')

        else:
            self.annot_path = os.path.join(
                self.data_dir, 'annotations', 'val_smoke.json')
        self.max_objs = 50
    ...
    def run_eval(self, results, save_dir):
        ...
        map=dets(detection_file='{}/results.json'.format(save_dir))
        return map
